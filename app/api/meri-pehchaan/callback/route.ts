...
import { meriPehchaanService } from "@/lib/meri-pehchaan-service"
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (error) {
      console.error("Meri Pehchaan authorization error:", error)
    ...
    const tokenResponse = await meriPehchaanService.getAccessToken(code)

    // Get user info
    const userInfo = await meriPehchaanService.getUserInfo(tokenResponse.access_token)
    ...
    console.error("Error in Meri Pehchaan callback:", error)
